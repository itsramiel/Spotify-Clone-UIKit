    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let viewModel = {
            ...
            case .playlist(model: let model):
                return SearchResultTableViewCellViewModel(artworkUrl: URL(string: model.images?.first?.url ?? ""), title: model.name, subtitle: model.owner.displayName ?? model.owner.id)
            }
        }()
